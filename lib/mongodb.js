...
const urlParser = require('mongo-url-parser');
...
MongoDB.prototype.connect = function(callback) {
  ...
  if (self.db) {
    ...
    new mongodb.MongoClient(self.settings.url, validOptions).connect(function(
      ...
      const url = urlParser(self.settings.url);
      if (err) {
        onError(err);
        return;
      }
...
MongoDB.prototype.execute = function(modelName, command) {
  ...
  if (self.client?.topology && !(self.client.topology.isDestroyed())) {
    doExecute();
  } else {
    if (self.db) {
      self.disconnect();
  ...
  function doExecute() {
    ...
    self.notifyObserversAround(
      ...
      async function(context, done) {
        const argsNoFunc = args.filter(arg => typeof arg !== 'function');

        debug('MongoDB: model=%s command=%s', modelName, command, argsNoFunc);

        try {
          const result = await collection[command].apply(collection, argsNoFunc);
          context.res = result;
          done(null, result);
        } catch (err) {
          done(err);
...
MongoDB.prototype.save = function(modelName, data, options, callback) {
  ...
  const dbData = self.toDatabase(modelName, data);

  this.execute(modelName, 'updateOne', {_id: oid}, {$set: dbData}, buildOptions({upsert: true},
    options), function(err, result) {
    if (!err) {
...
MongoDB.prototype.all = function all(modelName, filter, options, callback) {
  ...
  function processResponse(err, cursor) {
    ...
    const collation = options?.collation ?? filter?.collation;
    if (collation) {
...
function isObjectIDProperty(modelCtor, propDef, value, options) {
  ...
  if (typeof value === 'string' && value.match(ObjectIdValueRegex)) {
    ...
    return String(value)?.match(ObjectIdValueRegex);
  }
}
